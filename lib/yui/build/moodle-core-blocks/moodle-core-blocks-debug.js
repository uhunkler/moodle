...
MANAGER.prototype = {
    ...
    initializer : function() {
        ...
        // Add relevant classes and IDs to the 'content' block region on the My Home page.
        var myhomecontent = Y.Node.all('body#'+CSS.MYINDEX+' #'+CSS.REGIONMAIN);
        if (myhomecontent.size() > 0) {
            // If no "content" block region is found (a section or div
            // with the tag data-blockregion="content") then create the block region:
            // _ get the first region-main node
            // _ copy all child elements
            // _ create a new block region node
            // _ set all attributes on the node
            // _ remove the child elements from the DOM
            // _ append the child elements to the new node
            // _ append the new node to the region-main node
            if (!Y.one('[data-blockregion="content"]')) {
                Y.log('no data-blockregion="content" tag found');
                var contentwrapper = myhomecontent.item(0);
                var contentitems = contentwrapper.get('children');
                var contentregion = Y.Node.create('<div class="' + CSS.REGIONCONTENT + '"></div>');
                contentregion.addClass(CSS.BLOCKREGION);
                contentregion.set('id', 'block-' + CSS.REGIONCONTENT);
                contentregion.setAttribute('data-blockregion', 'content');
                contentregion.setAttribute('data-droptarget', '1');
                contentitems.remove();
                contentregion.append(contentitems);
                contentwrapper.append(contentregion);
            }
        }
